...
function StatCard({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="rounded-xl border p-4" style={{ backgroundColor: '#111118', borderColor: '#1e1e2e' }}>
      ...
      <p className="text-2xl font-bold font-mono" style={{ color: color ?? '#e2e8f0' }}>{value}</p>
    </div>
  )
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    tradeService.getStats().then(setStats).catch(() => {
      // Demo data when backend not running
      setStats({ totalTrades: 0, winRate: 0, totalPnl: 0, avgWin: 0, avgLoss: 0, profitFactor: 0, bestTrade: 0, worstTrade: 0 })
    })
    tradeService.getAll().then(setTrades).catch(() => setTrades([]))
  }, [])
 ...
 const pnlCurve = trades
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-xl border p-4" style={{ backgroundColor: '#111118', borderColor: '#1e1e2e' }}>
        <p className="text-sm mb-4" style={{ color: '#64748b' }}>Cumulative P&L</p>
        {pnlCurve.length > 1 ? (
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={pnlCurve}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: '#111118', border: '1px solid #1e1e2e', borderRadius: 8 }}
                labelStyle={{ color: '#64748b' }}
                itemStyle={{ color: '#00d4aa' }}
              />
              <Area type="monotone" dataKey="pnl" stroke="#00d4aa" fill="url(#pnlGrad)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
          <p className="text-center py-12 text-sm" style={{ color: '#64748b' }}>Ei vielä treidejä — lisää ensimmäinen!</p>
        )}
      </div>
    </div>
  )
